...
to quickly create a Cobra application.`,
	Run: func(cmd *cobra.Command, args []string) {
		...
		secret := os.Getenv("AVANZA_SECRET")
		credentials := authentication.AvanzaCredentials{
			Username: username,
			Password: password,
			Secret:   secret,
		}

		response := authentication.Authentication(credentials)

		os.Mkdir(".session", 0777)
		f, _ := json.MarshalIndent(response, "", " ")
		err := ioutil.WriteFile(".session/.data", f, 0666)
		fmt.Println(err)
	},
}
